...
pl_on_off %>% 
  ggplot(aes(PCT_AST_FGM, PCT_PTS_PAINT, colour = split,
             label = player)) +
  geom_point() +
  labs(title = "O estilo de jogo mudou ao nivel do jogador ?",
       subtitle = "Comparação com e sem capela no elenco") +
  ggrepel::geom_text_repel(data = pl_on_off %>%
                             filter(
                               PCT_AST_FGM >= 0.15 | PCT_AST_FGM <= -.08 |
                               PCT_PTS_PAINT >= .05 | PCT_PTS_PAINT <= - .09
                             )
                           )
```
