...
pl_long <- rockets_df %>% 
  ...
  group_by(player, variable, split) %>% 
  mutate(on_off_diff = diff(value)) %>% 
  ungroup()

capela <- pl_long %>% filter(str_detect(player, "Capela"))
```

Vamos pegar jogadores que jogaram com e sem o capela
```{r}
pl_w_and_wo_capela <- pl_long %>% group_by(player) %>% count() %>% filter(n>80) %>% pull(player)

df_w_wo_c <- pl_long %>% filter(player %in% pl_w_and_wo_capela) %>% 
  filter(on_off == "ON") %>% 
  select(- value, - on_off) %>% 
  pivot_wider(names_from = variable, values_from = on_off_diff)
